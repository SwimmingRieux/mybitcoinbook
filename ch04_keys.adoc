...
🔹 Trapdoor Function: A function that is easy to compute in one direction but can be reversed only with special knowledge (like a private key).
🔹 One-Way Hash Function: A function that is easy to compute but impossible (or infeasible) to reverse without brute force.
so basically, while you can produce a pubkey from a private key using a hash function, but you can't sign any message with it. signing is only the property of ECC.
...
noteforme: HASH160 is (SHA-256 followed by RIPEMD-160)
...
noteforme:
In Base58, when the version byte is 0x05, the resulting address starts with "3".
This is similar to how P2PKH addresses (with version 0x00) start with "1".
...
noteforme:
In a P2SH transaction with a 2-of-2 multisig redeem script, when Bitcoin full nodes execute the script, the stack looks like this before execution:
<signature2> <signature1> <redeem_script> OP_HASH160 <script_hash> OP_EQUAL

If the result of executing <redeem_script> OP_HASH160 <script_hash> OP_EQUAL> is 1, the stack then will look like this:
<signature2> <signature1> <redeem_script>
...
noteforme: https://github.com/SwimmingRieux/mybitcoinbook/blob/develop/ch04_keys_bech32.md

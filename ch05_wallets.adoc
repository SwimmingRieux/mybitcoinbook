...
noteforme: review all the pictures and noteformes.
...
noteforme: it also requires knowing what paths in the tree of keys your wallet application used for generating the specific keys it distributed.
...
noteforme: the two below pictures are extremely important.
...
noteforme: a good explaination can be found here: https://bitcoin.stackexchange.com/a/41598 \n
noteforme: so basically if you loose one of your private keys, the others would be safe. \n
noteforme: the reason we use HMAC-SHA512 function is this: we use SHA512 because we want to get a chaincode as entropy, because without it, one can derive all the private keys if they only find one of the private keys. so we need 512, left 256 ones are special integer, and the right 256 ones are chaincode. and we use HMAC because it slows down an attacker who has seen a part of the private key. read the below explaination which we saw before in this chapter of book which states:
...
noteforme:Each parent key can have 2,147,483,647 (231) children.
...
noteforme: in the previous noteformes we came to the conclusion that chaincode is necessay because if someone knows one of our private keys, would be able to derive all the other private keys. in the normal deriviation of pubkeys we have our chaincode on an online server(definetly private but it can get hacked), so now if someone knows one of our private keys and hack our chaincode, they would be able to derive all the other pr_keys. the hardened deriviation is differennt. because the formula is to use parent_private_key+chaincode+index rather than parent_public_key+chaincode+index. so now if someone knows one of our private keys and they also know our chaincode, they can't get all of our pr_keys. they can only know all the private keys in the subtree with the root that has used hardened deriviation. so you can make a hardened public key once at a while and give it to your server for generating its subtree, and its parents won't be in danger anymore.
...
noteforme: Instead of showing a large number like 2147483648 for the first hardened key, we display it as 0' (or 0h). Similarly, 2147483649 becomes 1', and so on.

...
noteforme: the reason we use HMAC-SHA512 function is this: we use SHA512 because we want to get a chaincode as entropy, because without it, one can derive all the private keys if they only find one of the private keys. so we need 51, left 256 ones are special integer, and the right 256 ones are chaincode. 

...
noteforme: for a quick recap, just take a look at pictures of this chapter, and read quickly what does each file do. also take this in mind, the witness data, is the data provided by the spender as a prrof that shows the nodes and miners that the spender is actually the owner of the output script.
...
noteforme: checkout variable length ints here: https://github.com/SwimmingRieux/mybitcoinbook/blob/develop/ch06_transactions_var_int.md
...
noteforme: a bitcoin node keeps a
database that stores every UTXO and
essential metadata about it (like its confirmation block height).  Each
time a new block of transactions arrives, all of the outputs they spend
are removed from the UTXO database and all of the outputs they create
...
noteforme: Another set of common terms is
_little-endian byte order_ for the internal version and _big-endian byte
order_ for the display version.
****
...
noteforme: Some users and businesses
objected because they used to accept unconfirmed transactions(in mempool) as payment of their costumers. but after RBF was proposed, people could invalidate their previous unconfirmed transaction by a new one to someone else, thuse leading to double-spending problem.
...
-- noteforme:
---"This script is not executed" → The coinbase field in a coinbase transaction is not executed like a normal script. Instead, it serves as a place where miners can include arbitrary data, such as extra nonce values, miner tags, or signaling information.

---"Legacy transaction limits on sigops do apply" → Even though the coinbase script is not executed, old Bitcoin rules still enforce limits on the number of signature operations ("sigops") inside transactions. This means that certain script patterns could still affect validation.

---"Any arbitrary data placed in it should be prefixed by a data-pushing opcode" → If miners want to include arbitrary data in the coinbase field, they should use an opcode like OP_PUSH to explicitly mark the data as non-executable. This prevents the script from accidentally being interpreted as something that violates legacy sigop limits.
...
noteforme: how did segwit increased the block size and also dis-incentivized the uneconomical outputs
